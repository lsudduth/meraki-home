...
def get_list_item_by_key(lst, key, value):
    """
    Return the first element of lst that has the given key (or dot-separated nested path) with the given value.
    ...
    >>> get_list_item_by_key([
        {"stack": {"id": "123", "name": "CoreStack"}, "vlanProfile": {"iname": "Profile1"}},
        {"stack": {"id": "456", "name": "AccessStack"}, "vlanProfile": {"iname": "Profile2"}},
    ...
    try:
        return next((item for item in lst if _get(item, keys) == value))
...
def validate_per_ssid_settings(api_data, my_data, ssids):
    ssids_numbers = {s["name"]: s["number"] for s in ssids}
