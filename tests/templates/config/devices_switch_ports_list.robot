...
{% for switch_ports_ranges in device.switch.ports | default([], true) %}
    {% for switch_ports_range in switch_ports_ranges.port_id_ranges %}
        {% for port_id in range(switch_ports_range.from | int, switch_ports_range.to | int + 1) %}
...
{% endif %}
Verify {{ organization.name }}/devices/{{ device.name }}/switch_ports/{{ switch_port.port_id }}//vlan_name{% if switch_port.vlan_name is defined %}
{% set ns = namespace(resolved_vlan=None) %}
{% for assignment in network.vlan_profiles_assignments | default([], true) %}
    {% if device.name in assignment.devices | default([], true) %}
        {% for profile in network.vlan_profiles | default([], true) %}
            {% if profile.iname == assignment.vlan_profile_iname %}
                {% for vname in profile.vlan_names | default([], true) %}
                    {% if vname.name == switch_port.vlan_name %}
                        {% set ns.resolved_vlan = vname.vlan_id %}
                    {% endif %}
                {% endfor %}
            {% endif %}
        {% endfor %}
    {% endif %}
{% endfor %}
{% if ns.resolved_vlan is none %}
    {% for profile in network.vlan_profiles | default([], true) %}
        {% if profile.iname == "Default" %}
            {% for vname in profile.vlan_names | default([], true) %}
                {% if vname.name == switch_port.vlan_name %}
                    {% set ns.resolved_vlan = vname.vlan_id %}
                {% endif %}
...
{% endif %}
    [Setup]   Get Meraki Data   /devices/{serial}/switch/ports/{{ switch_port.port_id }}   ['{{ organization.name }}', '{{ device.name }}']   switch_port
    Should Be Equal As Strings   ${switch_port}[vlan]   {{ ns.resolved_vlan }}

{% else %}
    Skip    switch_port.vlan_name is not defined
{% endif %}
...
{% endif %}
Verify {{ organization.name }}/devices/{{ device.name }}/switch_ports/{{ switch_port.port_id }}//voice_vlan_name{% if switch_port.voice_vlan_name is defined %}
{% set ns = namespace(resolved_vlan=None) %}
{% for assignment in network.vlan_profiles_assignments | default([], true) %}
    {% if device.name in assignment.devices | default([], true) %}
        {% for profile in network.vlan_profiles | default([], true) %}
...
{% endif %}
    [Setup]   Get Meraki Data   /devices/{serial}/switch/ports/{{ switch_port.port_id }}   ['{{ organization.name }}', '{{ device.name }}']   switch_port
    Should Be Equal As Strings   ${switch_port}[voiceVlan]   {{ ns.resolved_vlan }}

{% else %}
    Skip    switch_port.voice_vlan_name is not defined
{% endif %}
